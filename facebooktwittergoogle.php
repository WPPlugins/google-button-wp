...
add_action('init', 'London_time_2_init');

add_filter('the_content', 'London_time_2_content');

add_filter('the_excerpt', 'London_time_2_excerpt');

add_filter('plugin_action_links', 'London_time_2_add_settings_link', 10, 2 );

add_action('admin_menu', 'London_time_2_menu');

add_shortcode( 'London_time_2', 'London_time_2_shortcode' );

// PUBLIC FUNCTIONS

function London_time_2_init() {
	...
	$option = London_time_2_get_options_stored();

	if ($option['active_buttons']['facebook']==true) {
		wp_enqueue_script('London_time_2_facebook', 'http://static.ak.fbcdn.net/connect.php/js/FB.Share');
	}
	if ($option['active_buttons']['buzz']==true) {
		wp_enqueue_script('London_time_2_buzz', 'http://www.google.com/buzz/api/button.js');
	}
	if ($option['active_buttons']['google1']==true) {
		wp_enqueue_script('London_time_2_google1', 'http://apis.google.com/js/plusone.js');
	}
	if ($option['active_buttons']['twitter']==true) {
		wp_enqueue_script('London_time_2_twitter', 'http://platform.twitter.com/widgets.js');
	}
}    


function London_time_2_menu() {
	add_options_page('Facebook twitter google button Options', 'Facebook twitter google button', 'manage_options', 'London_time_2_options', 'London_time_2_options');
}


function London_time_2_add_settings_link($links, $file) {
	static $this_plugin;
	if (!$this_plugin) $this_plugin = plugin_basename(__FILE__);
 
	if ($file == $this_plugin){
		$settings_link = '<a href="admin.php?page=London_time_2_options">'.__("Settings").'</a>';
...
function London_time_2_content ($content) {
	return London_time_2 ($content, 'the_content');
}


function London_time_2_excerpt ($content) {
	return London_time_2 ($content, 'the_excerpt');
}


function London_time_2 ($content, $filter, $link='', $title='') {
	...
	if ($filter=='the_excerpt' and $last_execution=='the_content') {
		...
		remove_filter('the_content', 'London_time_2_content');
	...
	$custom_field_disable = get_post_custom_values('London_time_2_disable');
	...
	$out = '<div style="height:33px; padding-top:2px; padding-bottom:2px; clear:both;" class="London_time_2">';
	if ($option['active_buttons']['facebook']==true) {
		$first_shown = true;
		
		// REMOVE HTTP:// FROM STRING
		$facebook_link = (substr($link,0,7)=='http://') ? substr($link,7) : $link;
		$out .= '<div style="float:left; width:100px;" class="London_time_2_facebook"> 
	...
	if ($option['active_buttons']['facebook_like']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['facebook_like_width'].'px; '.$padding.'" class="London_time_2_facebook_like"> 
		...
		if ($option['facebook_like_send']) {
			...
			$out .= '<div style="float:left; width:50px; padding-left:10px;" class="London_time_2_facebook_like_send">
	...
	if ($option['active_buttons']['buzz']==true) {
		...
		$out .= '<div style="float:left; '.$padding.'" class="London_time_2_buzz"> 
	...
	if ($option['active_buttons']['google1']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['google1_width'].'px; '.$padding.'" class="London_time_2_google1"> 
	...
	if ($option['active_buttons']['twitter']==true) {
		...
		$out .= '<div style="float:left; width:'.$option['twitter_width'].'px; '.$padding.'" class="London_time_2_twitter"> 
...
function London_time_2_options () {

	$option_name = 'London_time_2';
	...
	if( isset($_POST['London_time_2_position'])) {
		$option = array();

		foreach (array_keys($active_buttons) as $item) {
			$option['active_buttons'][$item] = (isset($_POST['London_time_2_active_'.$item]) and $_POST['London_time_2_active_'.$item]=='on') ? true : false;
		}
		foreach (array_keys($show_in) as $item) {
			$option['show_in'][$item] = (isset($_POST['London_time_2_show_'.$item]) and $_POST['London_time_2_show_'.$item]=='on') ? true : false;
		}
		$option['position'] = esc_html($_POST['London_time_2_position']);
		$option['facebook_like_width'] = esc_html($_POST['London_time_2_facebook_like_width']);
		$option['facebook_like_text'] = ($_POST['London_time_2_facebook_like_text']=='recommend') ? 'recommend' : 'like';
		$option['facebook_like_send'] = (isset($_POST['London_time_2_facebook_like_send']) and $_POST['London_time_2_facebook_like_send']=='on') ? true : false;
		$option['google1_count'] = (isset($_POST['London_time_2_google1_count']) and $_POST['London_time_2_google1_count']=='on') ? true : false;
		$option['google1_width'] = esc_html($_POST['London_time_2_google1_width']);
		$option['twitter_count'] = (isset($_POST['London_time_2_twitter_count']) and $_POST['London_time_2_twitter_count']=='on') ? true : false;
		$option['twitter_width'] = esc_html($_POST['London_time_2_twitter_width']);
		$option['twitter_text'] = esc_html($_POST['London_time_2_twitter_text']);
	...
	$out .= '
	<style>
	#London_time_2_form h3 { cursor: default; }
	#London_time_2_form td { vertical-align:top; padding-bottom:15px; }
	</style>
	...
	<div style="float:left; width:74%; padding-right:1%;">
		...
		<form id="London_time_2_form" name="form1" method="post" action="">
		...
		<div class="inside">
			...
			foreach ($active_buttons as $name => $text) {
				...
				$out .= '<div style="width:250px; float:left;">
						<input type="checkbox" name="London_time_2_active_'.$name.'" '.$checked.' /> '
			...
			foreach ($show_in as $name => $text) {
				...
				$out .= '<div style="width:250px; float:left;">
						<input type="checkbox" name="London_time_2_show_'.$name.'" '.$checked.' /> '
						. __($text, 'menu-test' ).' &nbsp;&nbsp;</div>';
			...
			<td><select name="London_time_2_position">
		...
		<div class="inside">
			...
			<td>
				<input type="text" name="London_time_2_facebook_like_width" value="'.stripslashes($option['facebook_like_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 100", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Button text", 'menu-test' ).':</td>
			<td>
				<select name="London_time_2_facebook_like_text">
			...
			<td>
				<input type="checkbox" name="London_time_2_facebook_like_send" '.$facebook_like_show_send_button.' />
			</td></tr>
			</table>
		</div>
		</div>
		...
		<div class="inside">
			...
			<tr><td>'.__("Button width", 'menu-test' ).':</td>
			<td>
				<input type="text" name="London_time_2_google1_width" value="'.stripslashes($option['google1_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 90", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Show counter", 'menu-test' ).':</td>
			<td>
				<input type="checkbox" name="London_time_2_google1_count" '.$google1_count.' />
			</td></tr>
			</table>
		</div>
		</div>
		...
		<div class="inside">
			...
			<td>
				<input type="text" name="London_time_2_twitter_width" value="'.stripslashes($option['twitter_width']).'" size="10"> px<br />
				<span class="description">'.__("default: 110", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Additional text", 'menu-test' ).':</td>
			<td>
				<input type="text" name="London_time_2_twitter_text" value="'.stripslashes($option['twitter_text']).'" size="25"><br />
				<span class="description">'.__("optional text added at the end of every tweet, e.g. ' (via @authorofblogentry)'.
			   ", 'menu-test' ).'</span>
			</td></tr>
			<tr><td>'.__("Show counter", 'menu-test' ).':</td>
			<td>
				<input type="checkbox" name="London_time_2_twitter_count" '.$twitter_count.' />
			</td></tr>
			</table>
		</div>
		</div>
...
function London_time_2_shortcode ($atts) {
	return London_time_2 ('', 'shortcode');
}


//FUNCTION AVAILABLE FOR EXTERNAL INCLUDING INSIDE THEMES AND OTHER PLUGINS
function London_time_2_publish ($link='', $title='') {
	return London_time_2 ('', 'shortcode', $link, $title);
}
...
function London_time_2_get_options_stored () {
	//GET ARRAY OF STORED VALUES
	$option = get_option('London_time_2');
	 
	if ($option===false) {
		//OPTION NOT IN DATABASE, SO WE INSERT DEFAULT VALUES
		$option = London_time_2_get_options_default();
		add_option('London_time_2', $option);
	} else if ($option=='above' or $option=='below') {
		// Versions below 1.2.0 compatibility
		$option = London_time_2_get_options_default($option);
...
function London_time_2_get_options_default ($position='above') {
	$option = array();
